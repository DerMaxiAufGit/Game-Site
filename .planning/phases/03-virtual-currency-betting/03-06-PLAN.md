...
must_haves:
  ...
  artifacts:
    - path: "src/components/lobby/create-room-dialog.tsx"
      provides: "Updated room creation with free/bet toggle, bet amount presets, min/max bet inputs, payout ratio config"
      contains: "betAmount"
    - path: "src/components/lobby/room-card.tsx"
      provides: "Room card with chip icon and bet amount badge for bet rooms"
      contains: "Coins"
    - path: "server.js"
      provides: "Room creation handler accepting bet settings including minBet/maxBet, room data includes bet info"
      contains: "betAmount"
...
Purpose: Per user decisions, room creators can choose between free and bet rooms with full control over bet amounts, min/max bet, and payout ratios. The lobby must clearly distinguish bet rooms with chip icons and badge amounts, and provide filter tabs (All/Free/Bet).

Output: Updated room creation dialog with betting options including min/max bet, updated room cards with bet badges, lobby filter tabs, server-side room creation accepting bet settings.
</objective>
...
1. In RoomManager.createRoom():
   ...
   - Store in room object: isBetRoom (default false), betAmount (default 0), minBet (default 0), maxBet (default 0), payoutRatios (use default from SystemSettings if not provided)
...
2. In RoomManager.getPublicRooms():
   - Include isBetRoom, betAmount, minBet, maxBet, totalPot (betAmount * players.length for bet rooms, 0 for free), payoutRatios in the returned room info objects

3. In room:create handler:
   - Validate bet settings: if isBetRoom, betAmount must be > 0
   - If minBet or maxBet provided, validate: minBet <= betAmount <= maxBet (if both set)
...
`npx tsc --noEmit` passes. Create a room via the existing UI (without bet settings) - should still work with defaults (isBetRoom: false, betAmount: 0).
  ...
  <done>RoomSettings and RoomInfo types include betting fields (including minBet/maxBet). Server creates rooms with bet configuration. Room list includes bet info for lobby display. Existing free room flow still works.</done>
</task>

<task type="auto">
  <name>Task 2: Room creation dialog with betting options (including min/max bet) and lobby filter tabs</name>
...
3. **Min/Max bet inputs** (shown only when isBetRoom is true, below bet amount):
   - Per user decision: "Room creator can set min/max bet for their room"
   - Two number inputs side by side:
     - "Min. Einsatz" (optional, placeholder: "Kein Minimum")
     - "Max. Einsatz" (optional, placeholder: "Kein Maximum")
   - State: minBet (number | undefined), maxBet (number | undefined)
   - Validation: if both set, minBet <= maxBet. Show inline error if invalid.
   - These are optional room-level limits the creator can set to constrain bets in their room.

4. **Payout ratios section** (shown only when isBetRoom is true, collapsible/advanced):
...
5. Pass all bet settings (including minBet, maxBet) to room:create socket event.
...
- If room.isBetRoom:
  ...
  - If room.minBet or room.maxBet set, show small text: "Min: {minBet} / Max: {maxBet}" below bet badge
- If not bet room:
...
- Show count in each tab: "Alle (5)" | "Kostenlos (3)" | "Einsatz (2)"
  ...
  <verify>
`npx tsc --noEmit` passes. Start dev server, open lobby, create a bet room with preset amount and optional min/max. Verify room card shows chip badge (and min/max if set). Filter tabs switch between views. Create a free room, verify it shows "Kostenlos" badge. Both room types visible under "Alle" tab.
  </verify>
  <done>Room creation dialog has free/bet toggle with preset amounts, custom input, min/max bet inputs, and payout ratio config. Room cards show chip icon and bet amount badge for bet rooms (with optional min/max display). Lobby has filter tabs (All/Free/Bet).</done>
</task>
...
- Create bet room with min/max bet: shows min/max info on room card
...
<success_criteria>
Room creator can toggle free/bet, select preset or custom bet amount, set optional min/max bet for their room, configure payout ratios. Lobby clearly distinguishes bet rooms with visual badges. Filter tabs work. All settings stored on room and visible in lobby.
</success_criteria>
