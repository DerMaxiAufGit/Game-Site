...
must_haves:
  truths:
    ...
    - "User can send chips directly from any user's profile/player card via pre-filled transfer dialog"
  artifacts:
    ...
    - path: "src/components/wallet/transfer-dialog.tsx"
      provides: "Reusable transfer dialog that can be opened pre-filled with a recipient userId"
  ...
  key_links:
    ...
    - from: "src/components/wallet/transfer-form.tsx"
      to: "src/lib/actions/wallet.ts"
      via: "transferFunds server action, calls fetchBalance() after success"
      pattern: "transferFunds"
    - from: "src/components/wallet/transfer-dialog.tsx"
      to: "src/components/wallet/transfer-form.tsx"
      via: "Wraps TransferForm in a dialog with pre-filled recipient"
      pattern: "TransferForm"
---

<objective>
Full wallet page with balance chart, transaction history, daily claim, P2P transfers, and reusable transfer dialog.

Purpose: Per user decisions, the wallet page is the financial hub: balance graph at top, transaction list grouped by day with filters, daily allowance claim button, and a transfer form. This is the "PayPal for the platform" experience. Additionally, per user decision "Send money from wallet page OR directly from any user's profile/player card", a reusable TransferDialog component is created for use from player cards in other views.

Output: Wallet page at /wallet with all wallet features accessible from one page, plus a reusable TransferDialog that can be triggered from player cards anywhere in the app.
</objective>
...
<task type="auto">
  <name>Task 2: Transaction list, P2P transfer form, and reusable TransferDialog for player cards</name>
  <files>src/components/wallet/transaction-list.tsx, src/components/wallet/transfer-form.tsx, src/components/wallet/transfer-dialog.tsx</files>
  <action>
...
- Props: { prefillRecipientId?: string, prefillRecipientName?: string, onSuccess?: () => void }
  - When prefillRecipientId is provided, skip the user search and show the pre-filled recipient directly
- Form fields:
  1. Recipient: text input for username search (unless pre-filled)
...
- On success: toast "Transfer erfolgreich!", reset form, call fetchBalance() from socket provider to update sidebar balance in real-time, call onSuccess callback if provided
- On error: toast with error message
- If wallet is frozen: show warning and disable form
- Wrap in Card with title "Chips senden" (when used standalone on wallet page)

Create `src/components/wallet/transfer-dialog.tsx`:
- 'use client' component
- Per user decision: "Send money from wallet page OR directly from any user's profile/player card"
- Props: { recipientId: string, recipientName: string, trigger: React.ReactNode }
- Uses Radix Dialog (shadcn Dialog component)
- Trigger: the passed trigger element (e.g., a "Chips senden" button on a player card)
- Dialog content:
  - Title: "Chips an {recipientName} senden"
  - Body: renders TransferForm with prefillRecipientId={recipientId} and prefillRecipientName={recipientName}
  - On success: close dialog automatically
- Dark themed matching app: bg-zinc-900, border-zinc-800
- This component is designed to be imported from PlayerList, WaitingRoom, or any view that shows user info. Integration into player cards will be done in Plan 08 Task 2 alongside other player-related UI updates, or can be added to PlayerList/WaitingRoom directly.

Export TransferDialog for use by other components. Add a comment at the top of the file:
```
...
```
  ...
  <verify>
`npx tsc --noEmit` passes. Navigate to /wallet. Transaction list shows entries (may need to claim daily first to have data). Filter tabs filter correctly. Transfer form shows with user search (test by typing a username). Submit transfer updates both balances. TransferDialog component exists and is importable.
  </verify>
  <done>Transaction list with day grouping and filter tabs. Transfer form with user search, amount input, limit display, and fetchBalance() call after success for real-time sidebar update. Reusable TransferDialog component exists for use from player cards/profile views. Both integrated into wallet page. Transfers create matching transaction records for sender and receiver.</done>
</task>
...
- After successful transfer, sidebar balance updates immediately (fetchBalance() called)
- Sender and receiver balances update after transfer
- TransferDialog component is importable and renders a pre-filled transfer form in a dialog
- `npx tsc --noEmit` passes
</verification>

<success_criteria>
Full wallet page at /wallet with balance chart, daily claim with activity scaling, transaction history with filters, and P2P transfer form. Reusable TransferDialog component exists for triggering transfers from player cards. All operations create transaction records and update balance in real-time via fetchBalance().
</success_criteria>
